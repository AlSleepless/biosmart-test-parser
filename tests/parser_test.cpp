...
TEST(ParserTest, WaitForStart) {
  SimpleParser sp;
  std::vector<uint8_t> input_data = {0x01, 0x02, 0x03, 0x53, 0x01,
                                     0x07, 0x00, 0x02, 0x60, 0x43};
...
TEST(ParserTest, BzzPacket) {
  SimpleParser sp;
  std::vector<uint8_t> input_data = {0x53, 0x01, 0x0C, 0x00, 0x00, 0x6A,
                                     0x00, 0x02, 0x06, 0x06, 0x03, 0x25};
  ...
  auto packet = sp.get_packet();
  ASSERT_NE(packet, nullptr);
  ASSERT_STREQ(packet->to_string().c_str(),
               "type:BUZ,addr:1,sqn:0,reader:0,tone:2,on:6,off:6,count:3");
}

/*
  Неопределенные ситуации:
  1 Максимальный размера пакета (0xFFFF)
  2 Минимальный размера пакета (0x0000)
  3 Размер пакета не соответствует ожидаемому
  4 Адрес устройства больше 0x7E
